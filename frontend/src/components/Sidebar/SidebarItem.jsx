...
export const SidebarTitle = ({children}) => {
    return (
        <Box className="SidebarTitle">
            <Text weight="bold" size="2">{children}</Text>
        </Box>
    );
};
            ...
            <Tooltip.Root>
                <Tooltip.Trigger asChild>
                    <Box className={className} onClick={onClick}>
                        <Text size="2" highContrast>{record.label}</Text>
                    </Box>
                </Tooltip.Trigger>
        ...
        <Card className="SidebarItemTooltip">
            <DataList.Root size="1">
                ...
                <DataList.Item>
                    ...
                    <DataList.Value><Link href="">{record.node ? record.node.url : "missing"}</Link></DataList.Value>
                </DataList.Item>
                <DataList.Item>
